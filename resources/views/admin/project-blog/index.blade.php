        ...
        <div class="card">
            <div class="card-header header-part">
                <div class="row">
                    <div class="col-md-6 card_header_title">
                        <h3><i class="fa fa-gg-circle"></i> All Project</h3>
                    </div>
                    <div class="col-md-6 text-right card_header_btn">
                        <a href="{{url('admin/project-blog/add')}}" class="btn"><i class="fa fa-plus-circle"></i> Add
                            Project</a>
                    </div>
                </div>
            </div>
            <div id="printableTable" class="card-body table-responsive">
                <table cellspacing="0" bordercolor="gray" id="table_with_data"
                    ...
                    <tbody>
                        @php
                        $news = App\ProjectBlog::orderBy('id','DESC')->get();
                        @endphp
                        @foreach ($news as $data)
                        <tr>
                            
                            <td class="text-break">{{$data->project_title}}</td>
                            <td class="text-break">{{$data->photo_credit}}</td>
                            <td><img src="{{ asset('uploads/project-blog/'.$data->project_photo) }}"
                                    style="height: 50px;" alt="">
                            </td>
                            {{-- <td class="text-break">{!!Str::words($data->project_description,10)!!}</td> --}}
                            <td>
                                <div class="btn-group btn-group-sm btn-color-ceate">
                                    <a href="{{url('admin/project-blog/view',$data->id)}}"
                                        class="btn btn-info view-btn">View</a>
                                    <a href="{{url('admin/project-blog/edit',$data->id)}}"
...
<div class="modal fade" id="softDelModal" tabindex="-1" role="dialog" aria-labelledby="SoftDelModalTitle"
    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <form method="post" action="{{url('admin/project-blog/delete')}}">
            @csrf
